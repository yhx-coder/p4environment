...
#
#####################################################################################
# taken from behavioral model                                                       #
# see https://github.com/p4lang/behavioral-model/blob/master/tools/bmpy_utils.py    #
#####################################################################################
...
def check_JSON_md5(client, json_src, out=sys.stdout):
    ...
    if md5sum != bm_md5sum:
        my_print('**********\n')
        my_print("WARNING: the JSON files loaded into the switch and the one you just provided to this CLI don't have the same md5 sum.")
...
def thrift_connect(thrift_ip, thrift_port, services, out=sys.stdout):
    ...
    try:
        ...
        my_print('make sure the switch is running and that you have the right port\n')
        sys.exit(1)
...
def thrift_connect_standard(thrift_ip, thrift_port, out=sys.stdout):
    ...
    return thrift_connect(thrift_ip, thrift_port, [('standard', Standard.Client)], out)[0]
