...
#
######################################################################################
# adapted from behavioural model                                                     #
# see https://github.com/p4lang/behavioral-model/blob/master/tools/nanomsg_client.py #
######################################################################################
...
parser.add_argument('--thrift-ip', help=('thrift server IP address for table updates.'
                                         'if both --socket and --json are provided, then thrift will not be used.'),
                    type=str, action='store', required=False)
...
class NameMap:
    ...
    def load_names(self, json_cfg):
        ...
        for type_ in {'header_type', 'header', 'parser', 'deparser', 'action', 'pipeline', 'checksum'}:
...
class MSG_TYPES:
    ...
    def get_msg_class(type_):
        classes = {MSG_TYPES.PACKET_IN: PacketIn,
    ...
    @staticmethod
    def get_str(type_):
        strs = {MSG_TYPES.PACKET_IN: 'PACKET_IN',
...
class Msg(object):
    ...
    def __str__(self):
        return 'type: {}, switch_id: {}, cxt_id: {}, sig: {}, id: {}, copy_id: {}'.format(self.type_str, self.switch_id,
                                                                                          self.cxt_id, self.sig,
                                                                                          self.id_, self.copy_id)
...
class PacketIn(Msg):
    ...
    def __str__(self):
        return super(PacketIn, self).__str__() + ', port_in: {}'.format(self.port_in)
...
class PacketOut(Msg):
    ...
    def __str__(self):
        return super(PacketOut, self).__str__() + ', port_out: {}'.format(self.port_out)
...
class ConfigChange(Msg):
    ...
    def __str__(self):
        return 'type: {}, switch_id: {}'.format(self.type_str, self.switch_id)
...
def recv_msgs(socket_addr, client):
    ...
    while True:
        ...
        if p.type_ == MSG_TYPES.CONFIG_CHANGE:
            ...
            print('requesting new config from switch, which may cause some log messages to be dropped')
            json_init(client)
...
def main():
    ...
    for a in deprecated_args:
        if getattr(args, a) is not None:
            print("command line option '--{}' is deprecated and will be ignored".format(a))
    ...
    if not args.json or not args.socket:
        ...
        if info.elogger_socket is None:
            print("the event logger is not enabled on the switch, run with '--nanolog <addr>'")
            sys.exit(1)
        if args.socket is None:
            socket_addr = info.elogger_socket
            print("'--socket' not provided, using", socket_addr, '(obtained from switch)')
