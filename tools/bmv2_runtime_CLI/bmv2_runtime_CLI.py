...
def get_parser():
    ...
    parser.add_argument('--thrift-port', help='thrift server port for table updates',
                        type=int, action='store', default=4221)
...
def parse_runtime_data(action, params):
    def parse_param_(field, bw):
        try:
            ...
            raise UIn_RuntimeDataError('error while parsing {} - {}'.format(field, e))
...
def parse_match_key(table, key_fields):
    def parse_param_(field, bw):
        try:
            ...
            raise UIn_MatchKeyError('error while parsing {} - {}'.format(field, e))
...
def deprecated_act_prof(substitute, with_selection=False,
    ...
    def deprecated_act_prof_(f):
        # not sure if this is the right place for it, if I want it to play nice with @wraps
...
class RuntimeAPI(cmd.Cmd):
    ...
    def exactly_n_args(self, args, n):
        if len(args) != n:
            raise UIn_Error('wrong number of args, expected {} but got {}'.format(n, len(args)))
    ...
    def do_table_set_default(self, line):
        ...
        if len(args[2:]) != action.num_params():
            raise UIn_Error('action {} needs {} parameters'.format(action_name, action.num_params()))
    ...
    def do_table_add(self, line):
        ...
        entry_handle = self.client.bm_mt_add_entry(0, table.name, match_key, action.name, runtime_data,
    ...
    def do_mc_dump(self, line):
        ...
        for mgrp in mc_json['mgrps']:
            ...
            for L1h in mgrp['l1_handles']:
                ...
                print('-> (ports=[{}], lags=[{}])'.format(', '.join([str(p) for p in ports]),
    ...
    def do_counter_read(self, line):
        ...
        print('{}[{}]= '.format(counter_name, index), value)
    ...
    # notice the strictly_deprecated=False;
    # I don't consider this command to be strictly deprecated because it can be convenient and
    # does not modify the action profile so won't create problems
